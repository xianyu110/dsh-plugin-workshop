...
| 安装插件 | 详情页复制安装命令手动执行（一键安装见 Roadmap） |
| 提高搜索额度 | 工具栏 ⚙ → 填入 GitHub Personal Access Token |

## 🏗️ 架构

- **宿主侧**：无（纯客户端插件，`lib/index.js` 为空实现）
...
- [ ] v1.1 一键安装/更新（宿主侧 API：git 探测、clone/pull 到 `.agent-presets`，需要本机 git）
- [ ] 已安装插件管理页
- [ ] 申请加入 awesome-dsh-plugin / awesome-dsh-plugins 等社区索引
