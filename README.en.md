...
- **Smart install / uninstall with pre-flight grading**: the detail page probes the repo structure up front and shows a three-level verdict — installable / risky / manual-only. Standard bundle/nested/preset shapes keep one-click install; special shapes (multi-package monorepos, non-plugin repos, build scripts) degrade to **the author's own install instructions** (commands extracted from the README, copyable, never auto-executed). Uninstall reverses the install path (no files are touched when pnpm remove fails — no dangling links; on success dsh web restarts automatically to refresh the plugin table); missing git is reported with clear guidance
