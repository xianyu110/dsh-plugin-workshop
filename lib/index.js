/**
 * dsh-plugin-workshop（宿主侧）v1.4.4
 *
 * 智能一键安装/卸载：
    ...
    for (const s of subs) {
      ...
      if (existsSync(pp)) {
        try {
          ...
          if (pkg && pkg.dsh && typeof pkg.name === 'string' && pkg.name) found.push({ dir: childRel, name: pkg.name })
        } catch (e) { /* 非 JSON */ }
      }
    ...
    handler: async (req, res) => {
      ...
      try {
        if (path === API + '/status') {
          ...
          return sendJson(res, 200, { ok: true, dir, list, git: gitAvailable(), rows: readPatchRows(), deps: profileDeps(), bundles: profileBundles(), installed: buildInstalled() })
        }
        ...
        if (path === API + '/install') {
          ...
          if (type === 'preset') {
            // 嵌套包检测：根目录无 dsh 声明，但目录树中恰好一个 dsh 包
            // （皮肤合集 / packages/<name>/ 等 monorepo 布局，递归最深 4 层）
            const candidates = collectDshPackages(tmp)
