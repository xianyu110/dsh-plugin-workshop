/**
 * dsh-plugin-workshop（宿主侧）v1.2.1
 *
 * 智能一键安装/卸载：
 * - GET /dsh-plugin-workshop/api/status    → 插件目录、已安装列表、profile 激活行、git 探测
 * - GET /dsh-plugin-workshop/api/install   → 临时克隆检测类型：
 *     bundle 型（package.json 声明 dsh.*）→ dsh plugin add github:... + 写 profile 激活行
 *     preset 型（无 dsh 声明）            → 拷入 .agent-presets/<repo>
 * - GET /dsh-plugin-workshop/api/update    → bundle 型 pnpm update；preset 型 git pull
 * - GET /dsh-plugin-workshop/api/uninstall → 按安装方式逆向清理（删激活行+pnpm remove / 删目录）
 *
 * 安全：
    ...
    handler: async (req, res) => {
      ...
      try {
        if (path === API + '/status') {
          ...
          return sendJson(res, 200, { ok: true, dir, list, git: gitAvailable(), rows: readPatchRows() })
        }
        ...
        if (path === API + '/uninstall') {
          ...
          if (row) {
            ...
            let removeMsg = ''
            if (row.name) {
              const rr = runCmd('cmd', ['/c', 'dsh', 'plugin', '--profile', 'web', 'remove', row.name], 240000)
              if (rr.status !== 0) removeMsg = '；pnpm remove 失败：' + String(rr.stderr || rr.stdout || '').slice(0, 200)
            }
            ...
            return sendJson(res, 200, { ok: true, removed: 'bundle', note: '激活行已移除' + removeMsg + '。需要重启 dsh web 后完全生效。' })
          }
          const target = join(presetsDir(), repoName)
          if (existsSync(target)) {
            rmSync(target, { recursive: true, force: true })
            return sendJson(res, 200, { ok: true, removed: 'preset', path: target })
          }
          return sendJson(res, 404, { ok: false, error: '未找到该插件的安装记录' })
        }
        ...
        if (path === API + '/install') {
          ...
          if (type === 'nested') {
            ...
            return sendJson(res, 200, { ok: true, type: 'nested', pkgName, path: target, note: '已按嵌套包方式安装（link 到子包 ' + nestedDir + ' + 激活行）。需要重启 dsh web 后生效。' })
          }
          if (type === 'bundle') {
            rmSync(tmp, { recursive: true, force: true })
            ...
            if (!readPatchRows().some((r) => r.id === repoName)) appendPatchRow(repoName, pkgName)
            return sendJson(res, 200, { ok: true, type: 'bundle', pkgName, note: '已按 bundle 方式安装（dsh plugin add + 激活行）。需要重启 dsh web 后生效。' })
          }
          const dir = presetsDir()
          try { mkdirSync(dir, { recursive: true }) } catch (e) {
            return sendJson(res, 500, { ok: false, error: '无法创建插件目录：' + dir })
        ...
        if (path === API + '/update') {
          ...
          if (row) {
            ...
            const ru = runCmd('cmd', ['/c', 'dsh', 'plugin', '--profile', 'web', 'update', row.name || repoName], 300000)
            if (ru.status !== 0) {
