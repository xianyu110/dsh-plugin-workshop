/**
 * dsh-plugin-workshop（宿主侧）v1.6.2
 *
 * 智能一键安装/卸载：
...
function removePatchRow(id) {
  ...
  // 若已无任何数组项（只剩注释/空行），写回明确的空数组占位，
  // 避免 YAML 解析成 null 导致 Harness 启动崩溃。
  const hasItem = lines.some((l) => /^\s*-\s+/.test(l))
  if (!hasItem) {
...
/** 去掉文档末尾的空数组占位（[]），以便追加 insert 块。 */
function stripEmptyArrayPlaceholder(text) {
  const lines = String(text || '').split(/\r?\n/)
  for (let i = lines.length - 1; i >= 0; i--) {
    const t = lines[i].trim()
    if (t === '') continue
    if (t === '[]') lines.splice(i, 1)
    break
  }
  return lines.join('\n')
}
...
function appendPatchRow(id, name) {
  ...
  text = stripEmptyArrayPlaceholder(text)
  if (text && !/\n$/.test(text)) text += '\n'
  text += '- insert:\n    - id: ' + id + "\n      name: '" + name + "'\n"
 ...
 */
/** 是否运行在桌面 App（Electron）内——此时杀 process.pid 会关掉整个 App 窗口。 */
function isDesktopApp() {
  return !!(typeof process !== 'undefined' && process.versions && process.versions.electron)
}

/** 安装/更新后的重启提示文案（桌面 App 与命令行 dsh 措辞不同）。 */
function restartHint() {
  return isDesktopApp() ? '需要重启 DeepSeek Harness App 后生效' : '需要重启 dsh web 后生效'
}

function scheduleRestart() {
  if (process.platform !== 'win32') return false
  if (isDesktopApp()) return false // 桌面 App：不自动自杀，改为提示用户重启 App
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (path === API + '/uninstall') {
          ...
          const tail = restarted ? '。dsh web 将在几秒后自动重启以刷新插件表（页面会自动恢复）' : (isDesktopApp() ? '。需要重启 DeepSeek Harness App 后完全生效。' : '。需要重启 dsh web 后完全生效。')
        ...
        if (path === API + '/install') {
          ...
          if (type === 'nested') {
            ...
            return sendJson(res, 200, { ok: true, type: 'nested', pkgName, path: target, note: '已按嵌套包方式安装（link 到子包 ' + nestedDir + '）。' + actNote + '。' + restartHint() + '。' })
          }
          if (type === 'bundle') {
            rmSync(tmp, { recursive: true, force: true })
            ...
            const actNote = activateAfterAdd(pkgName, repoName)
            reportEvent(full, 'install')
            return sendJson(res, 200, { ok: true, type: 'bundle', pkgName, note: '已按 bundle 方式安装（dsh plugin add）。' + actNote + '。' + restartHint() + '。' })
          }
          const dir = presetsDir()
          try { mkdirSync(dir, { recursive: true }) } catch (e) {
            return sendJson(res, 500, { ok: false, error: '无法创建插件目录：' + dir })
        ...
        if (path === API + '/update') {
          ...
          if (depName) {
            ...
            if (cloneDir && existsSync(cloneDir)) {
              ...
              return sendJson(res, 200, { ok: true, note: '已拉取最新代码（link 安装），' + restartHint() + '。' })
            }
            ...
            return sendJson(res, 200, { ok: true, note: '已更新依赖；' + restartHint() + '。' })
          }
